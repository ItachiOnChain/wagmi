...
contract TokenFactory {
    ...
    function buy( address tokenAddress, uint amount) external payable{
        require(tokens[tokenAddress] == TokenState.ICO, "Token doesnt exist or not in available ICO");
    ...
    function withdraw(address tokenAddress, address to) external {
        require(tokens[tokenAddress] == TokenState.TRADING, "Token Doesnt exist or hasent reached funding goal yet");
    ...
    function sell(address tokenAddress, uint amount) external {
        ...
        require(balances[tokenAddress][msg.sender] >= amount, "Not enough tokens to sell");
    ...
    function calculateEthForSell(address tokenAddress, uint amount) public view returns (uint) {
        Token token = Token(tokenAddress);
        uint a = token.totalSupply() - INITIAL_MINT;
        uint b = a - amount;
        uint f_a = k * a + offset;
